...
from shadowgirl import BOT_USERNAME, MUSIC_BOT_NAME, app, db_mem
from shadowgirl.Core.PyTgCalls import Queues, shadowgirl
from shadowgirl.Core.PyTgCalls.Converter import convert
from shadowgirl.Core.PyTgCalls.Downloader import download
from shadowgirl.Database import (_get_playlists, delete_playlist, get_playlist,
                            get_playlist_names, is_active_chat, save_playlist)
from shadowgirl.Database.queue import (add_active_chat, is_active_chat,
                                  is_music_playing, music_off, music_on,
                                  remove_active_chat)
from shadowgirl.Decorators.admins import AdminRightsCheckCB
from shadowgirl.Decorators.checker import checkerCB
from shadowgirl.Inline import (audio_markup, audio_markup2, download_markup,
                          fetch_playlist, paste_queue_markup, primary_markup)
from shadowgirl.Utilities.changers import time_to_seconds
from shadowgirl.Utilities.chat import specialfont_to_normal
from shadowgirl.Utilities.paste import isPreviewUp, paste_queue
from shadowgirl.Utilities.theme import check_theme
from shadowgirl.Utilities.thumbnails import gen_thumb
from shadowgirl.Utilities.timer import start_timer
from shadowgirl.Utilities.youtube import get_yt_info_id
...
async def admin_risghts(_, CallbackQuery):
    ...
    if command == "pausecb":
        ...
        await shadowgirl.pytgcalls.pause_stream(chat_id)
    ...
    if command == "resumecb":
        ...
        await shadowgirl.pytgcalls.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
    ...
    if command == "stopcb":
        ...
        await shadowgirl.pytgcalls.leave_group_call(chat_id)
        await CallbackQuery.message.reply_text(
    ...
    if command == "skipcb":
        ...
        if Queues.is_empty(chat_id):
            ...
            if str(finxx) != "raw":
                ...
                await shadowgirl.pytgcalls.change_stream(
                    chat_id,
                    InputStream(
                        InputAudioStream(
                            raw_path,
    ...
    if not _playlist:
        ...
        for shikhar in _playlist:
            ...
            if await is_active_chat(chat_id):
                ...
                raw_path = await convert(downloaded_file)
                try:
                    await shadowgirl.pytgcalls.join_group_call(
                        chat_id,
                        InputStream(
                            InputAudioStream(
                                raw_path,
