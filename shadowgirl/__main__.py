...
from shadowgirl import (ASSID, ASSMENTION, ASSNAME, ASSUSERNAME, BOT_ID, BOT_NAME,
                   BOT_USERNAME, SUDOERS, app, db, userbot)
from shadowgirl.Core.Logger.Log import (startup_delete_last, startup_edit_last,
                                   startup_send_new)
from shadowgirl.Core.PyTgCalls.shadowgirl import run
from shadowgirl.Database import get_active_chats, get_sudoers, remove_active_chat
from shadowgirl.Inline import private_panel
from shadowgirl.Plugins import ALL_MODULES
from shadowgirl.Utilities.inline import paginate_modules
...
async def initiate_bot():
    with console.status(
        "[magenta] Booting up The shadowgirl Music Bot...",
    ) as status:
    ...
    console.print(
        "[bold green]Congrats!! shadowgirl Music Bot has started successfully!\n"
    )
    try:
    ...
    try:
        ...
        return
    try:
        await userbot.join_chat("Officialshadowgirl")
    except:
        pass
